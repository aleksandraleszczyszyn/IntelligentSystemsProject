...
Memory::Memory(){
    ...
    //fixedSearchRadius_L2 = 0.22;

    fixedSearchRadius_L2 = 0.55;
    ...
    featureRedundancyTreshold_L2 = 0.011;
...
void Memory::checkRedundancy_L2(vector<int>& featsIdx)
{
	if(!forgetFeatures)
		return;

	int categoryId = getFeatureMetadata(featsIdx.at(0)).getCategory();
	double distance;
	int featId;
	const pcl::Histogram<153>* si;
	FeatureMetadata *aFeatureMetadata;
	int clusterId = 0;
	//int currClusterId = 0;
	Cluster *cluster;
	unordered_set<int> *cFeatures;
	stringstream lookupKey;

	//cout << "000 " << endl;

	for(size_t i = 0; i < featsIdx.size(); i++)
	{
		featId = featsIdx.at(i);
		aFeatureMetadata = &getFeatureMetadata(featId);

		si = &aFeatureMetadata->getFeature();
		clusterId = aFeatureMetadata->getCluster();

		cluster = &clusters[clusterId];
		cFeatures = &cluster->getPoints();

		for(unordered_set<int>::iterator it = (*cFeatures).begin(); 
						 it != (*cFeatures).end(); ++it) 
		{
			if(featId == *it)	
				continue;
			...
			distance = searcher.euclideanDistance(*si, getFeatureMetadata(*it).getFeature());
			...
			lookupTable[lookupKey.str()] = distance;

			//cout << "--- " << endl;

		        if(distance <= featureRedundancyTreshold_L2)
			{
				redistributeWeights_L2(featId, si, fixedSearchRadius_L2, categoryId);

				// remove features from memory
				remFeatureFromMemory(featId, getFeatureMetadata(featId));

				numRedundantFeatures +=1;
				break;
			}
		}
...
/*
		...
		{
			unordered_map<string, double> lookupTable;

			lookupKey.str("");
			lookupKey.clear();

			distance = searcher.euclideanDistance(*si, it->second.getFeature());

			lookupKey << featId << "-" << it->first;

			lookupTable[lookupKey.str()] = distance;

		        if(distance != 0 && distance <= featureRedundancyTreshold_L2)
			{
				//remFeatureFromMemory2(featId, *aFeatureMetadata);

				redistributeWeights_L2(featId, si, fixedSearchRadius_L2, 										 categoryId, lookupTable);

				//redistributeLocalWeights_L2(featId, si, categoryId);;

				// remove features from memory
				remFeatureFromMemory(featId, getFeatureMetadata(featId));
				//removeFeatureFromMemory(featId);
...
void Memory::applyMemoryDecay_L2()
{
	numSeenObjects++;
    	...
    	for(unordered_map<int, FeatureMetadata>::iterator it = featuresMetadata.begin(); it != featuresMetadata.end(); ++it) 
	{
		...
		// forget this feature
		if(newFeatureGlobalWeight <= memoryDecayTreshold)
		{		
			redistributeWeights_L2(	it->first, 
						&it->second.getFeature(), 
						fixedSearchRadius_L2, 
						it->second.getCategory());

			// remove features from memory
			remFeatureFromMemory(it->first, it->second);

			nForgottenFeaturesByMemoryDecayFactor++;
		}
    	}	
...
/*
void Memory::redistributeLocalWeights_L2(int toForget, const pcl::Histogram<153>* si, int categoryId)
{
	...
	double localWeight = getFeatureMetadata(toForget).getLocalWeight();
	...
	for(unordered_set<int>::iterator it = cFeatures.begin(); it != cFeatures.end(); ++it) 
	{
		if(toForget != *it)
		{
			...
			sumOfDistances += 1 / sqrt(distance);
		}
	}


	cout << "test2 " << sumOfDistances << endl;
	...
	for(unordered_set<int>::iterator it = cFeatures.begin(); it != cFeatures.end(); ++it) 
	{
		if(toForget != *it)
		{
			aFeatureMetadata = &featuresMetadata[*it];

			newLocalWeight = aFeatureMetadata->getLocalWeight() 
					+ (1 / sqrt(distances.at(i)) / sumOfDistances)
					* localWeight;
...
void Memory::redistributeWeights_L2(int toForget, const pcl::Histogram<153>* si, double searchRadius, int categoryId)
{
	double sumOfDistances = 0;
	double sumOfDistances2 = 0;
	...
	double globalWeight = getFeatureMetadata(toForget).getGlobalWeight();
	double localWeight = getFeatureMetadata(toForget).getLocalWeight();
						  	  ...
						  	  it != featuresMetadata.end(); ++it) 
	{
		...
		{	
			...
			if(toForget < featureId)
				lookupKey << toForget << "-" << featureId;
			else
				lookupKey << featureId << "-" << toForget;
		
			unordered_map<string, double>::iterator it2 = lookupTable.find(lookupKey.str());

			if(it2 == lookupTable.end())
			{
				distance = searcher.euclideanDistance(*si, it->second.getFeature());
				lookupTable.erase(lookupKey.str());
				//lookupTable.erase(it2);
			}
			else
				distance = it2->second;
			// End New.
			...
			if(distance <= searchRadius)
			{
				...
				if(it->second.getCategory() == categoryId)
				{
					sumOfDistances2 += 1 / sqrt(distance);
				}
			}
			else if(it->second.getCategory() == categoryId)
			{
				distances2.push_back(distance);
				sumOfDistances2 += 1 / sqrt(distance);
				mapping2.push_back(it->first);
			}
		}	
	}
	...
	{
		aFeatureMetadata = &featuresMetadata[mapping2.at(j)];

		newFeatureWeight = aFeatureMetadata->getLocalWeight() 
				+ (1 / sqrt(distances2.at(j)) / sumOfDistances2)
				* localWeight;

		aFeatureMetadata->setLocalWeight(newFeatureWeight);	
	}

	for(size_t j = 0; j < mapping.size(); j++)
	{
		aFeatureMetadata = &featuresMetadata[mapping.at(j)];
		
		if(aFeatureMetadata->getCategory() == categoryId)
		{
			newFeatureWeight = aFeatureMetadata->getLocalWeight() 
					+ (1 / sqrt(distances.at(j)) / sumOfDistances2)
					* localWeight;
		...
		newFeatureGlobalWeight = featureGlobalWeight 
				       + ((1 / sqrt(distances.at(j))) / sumOfDistances)
				       * globalWeight;
...
void Memory::checkRedundancy(vector<int>& featsIdx)
{
	...
	for(size_t i = 0; i < featsIdx.size(); i++){
		...
		//Searcher s;
		/*
		si = &getFeatureMetadata(featsIdx.at(i)).getFeature();
		cout << "Found: " << foundKnn << endl;
		for(size_t j = 0; j < knnDistances.size(); j++)
		{
			cout << knnIds.at(j) << " " << knnDistances.at(j) << endl;
		}

		for(size_t j = 0; j < knnDistances.size(); j++)
		{
		   cout << knnIds.at(j) << " " 
			<< searcher.euclideanDistance(*si, getFeatureMetadata(knnIds.at(j)).getFeature()) 
			<< endl;
		}
...
/*
void Memory::remFeatureFromMemory2(int featureId, FeatureMetadata &featureMetadata)
{
	// update cluster centroid
	Cluster &cluster = clusters[featureMetadata.getCluster()];
