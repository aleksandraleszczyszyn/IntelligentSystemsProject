...
.		* @param[in] featsIdx features ids to assign
		...
		*/
		...
		int getLookupTableSize() { return lookupTable.size();}

		double getSearchRadius() { return fixedSearchRadius_L2;}

		double getRedundancyTreshold() { return featureRedundancyTreshold_L2;}
		...
		void checkRedundancy_L2(vector<int>& featsIdx);

		void applyMemoryDecay_L2();

		void redistributeWeights_L2(int toForget, const pcl::Histogram<153>* si, double searchRadius, int categoryId);

		void redistributeLocalWeights_L2(int toForget, const pcl::Histogram<153>* si, int categoryId);

		void remFeatureFromMemory2(int featureId, FeatureMetadata &featureMetadata);
    ...
    protected:
		...
		double featureRedundancyTreshold_L2;
